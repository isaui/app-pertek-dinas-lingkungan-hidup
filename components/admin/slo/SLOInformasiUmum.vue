<template>
    <div class="space-y-6">
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <dd class="mt-1 text-sm text-gray-900">{{ slo.company }}</dd>
          </div>
          <div>
            <dt class="text-sm font-medium text-gray-500">Alamat</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ slo.address }}</dd>
          </div>
          <div>
            <dt class="text-sm font-medium text-gray-500">Jenis SLO</dt>
            <dd class="mt-1">
              <span v-if="slo.type === 'EMISI'" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                Pembuangan Emisi
              </span>
              <span v-else-if="slo.type === 'AIR_LIMBAH'" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                Air Limbah
              </span>
            </dd>
          </div>
          <div>
            ...
            <dd class="mt-1">
              <span :class="getStatusClass(slo.status)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium">
                {{ getStatusLabel(slo.status) }}
              </span>
            </dd>
          </div>
          <div v-if="slo.sloNumber">
            <dt class="text-sm font-medium text-gray-500">Nomor SLO</dt>
            <dd class="mt-1 text-sm font-mono text-gray-900 bg-green-50 px-2 py-1 rounded">{{ slo.sloNumber }}</dd>
          </div>
          <div>
            <dt class="text-sm font-medium text-gray-500">Tanggal Pengajuan</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ formatDate(slo.createdAt) }}</dd>
          </div>
          <div v-if="slo.approvedAt">
            <dt class="text-sm font-medium text-gray-500">Tanggal Disetujui</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ formatDate(slo.approvedAt) }}</dd>
          </div>
        </dl>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            ...
            <dd class="mt-1 text-sm text-gray-900">{{ slo.user?.name || 'Tidak tersedia' }}</dd>
          </div>
          <div>
            <dt class="text-sm font-medium text-gray-500">Email</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ slo.user?.email || 'Tidak tersedia' }}</dd>
          </div>
          <div v-if="slo.user?.instansi">
            <dt class="text-sm font-medium text-gray-500">Instansi</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ slo.user.instansi }}</dd>
          </div>
          <div v-if="slo.user?.nomorHp">
            <dt class="text-sm font-medium text-gray-500">Nomor HP</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ slo.user.nomorHp }}</dd>
          </div>
        </dl>
      </div>
  
      <!-- Informasi Verifikasi Lapangan (jika ada) -->
      <div v-if="slo.verifikasiDate || slo.verifikasiLocation" class="bg-white border border-gray-200 rounded-lg p-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Informasi Verifikasi Lapangan</h3>
        <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div v-if="slo.verifikasiDate">
            <dt class="text-sm font-medium text-gray-500">Tanggal & Waktu</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ formatDateTime(slo.verifikasiDate) }}</dd>
          </div>
          <div v-if="slo.verifikasiLocation">
            <dt class="text-sm font-medium text-gray-500">Lokasi</dt>
            <dd class="mt-1 text-sm text-gray-900">{{ slo.verifikasiLocation }}</dd>
          </div>
        </dl>
      </div>
  
      <!-- Catatan (jika ada) -->
      <div v-if="slo.notes || slo.feedbackToUser" class="bg-white border border-gray-200 rounded-lg p-6">
        <h3 class="text-lg font-medium text-gray-900 mb-4">Catatan</h3>
        <div class="space-y-4">
          <div v-if="slo.notes">
            <dt class="text-sm font-medium text-gray-500">Catatan Pemohon</dt>
            <dd class="mt-1 text-sm text-gray-900 bg-gray-50 p-3 rounded-md">{{ slo.notes }}</dd>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-5">
          ...
          <div class="text-center">
            <div class="text-2xl font-bold text-purple-600">{{ getDocumentsByType('SURAT_UNDANGAN_VERIFIKASI').length }}</div>
            <div class="text-sm text-gray-500">Surat Undangan</div>
          </div>
          <div class="text-center">
            <div class="text-2xl font-bold text-green-600">{{ getDocumentsByType('SLO_FINAL').length }}</div>
  ...
  const getDocumentsByType = (type: string) => {
    return props.slo.documents?.filter((doc: any) => doc.type === type) || []
  }
  </script>