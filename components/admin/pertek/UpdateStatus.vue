      ...
      <form @submit.prevent="submitStatusUpdate" class="space-y-6">
        ...
        <div v-if="needsRequirementFeedback(formData.newStatus)" class="border-t pt-6">
          <h4 class="text-lg font-medium text-gray-900 mb-4">
            <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Feedback Hasil Paparan</span>
            <span v-else>Feedback Persyaratan Detail</span>
          </h4>
          <p class="text-sm text-gray-600 mb-4">
            <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">
              Berikan feedback spesifik berdasarkan hasil paparan untuk membantu pemohon memahami hasil evaluasi dan tindak lanjut yang diperlukan.
            </span>
            <span v-else>
          ...
          <div class="space-y-4">
            <div v-for="(feedback, index) in formData.requirementFeedbacks" :key="index" 
              ...
              <div class="flex justify-between items-start mb-3">
                <h5 class="text-sm font-medium text-gray-900">
                  <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Feedback Paparan {{ index + 1 }}</span>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">
                    <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Kategori Feedback</span>
                  ...
                  <select v-model="feedback.requirementType" 
                          ...
                          required>
                    <option value="">
                      <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Pilih kategori feedback</span>
                      <span v-else>Pilih jenis persyaratan</span>
                    </option>
                    <template v-if="formData.newStatus === 'PAPARAN_COMPLETED'">
                      <option value="PAPARAN_TEKNIS">Aspek Teknis</option>
                      <option value="PAPARAN_LINGKUNGAN">Aspek Lingkungan</option>
                      <option value="PAPARAN_SOSIAL">Aspek Sosial</option>
                      <option value="PAPARAN_EKONOMI">Aspek Ekonomi</option>
                      <option value="PAPARAN_METODE">Metodologi</option>
                      <option value="PAPARAN_DATA">Data dan Analisis</option>
                      <option value="PAPARAN_MITIGASI">Mitigasi dan Pengendalian</option>
                      <option value="PAPARAN_MONITORING">Monitoring dan Evaluasi</option>
                      <option value="PAPARAN_OTHER">Lainnya</option>
                    </template>
                    <template v-else>
                ...
                <div>
                  ...
                  <select v-model="feedback.documentId" 
                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    <option value="">
                      <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Feedback umum paparan</span>
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                  <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">Detail Feedback Paparan</span>
                  <span v-else">Detail Feedback</span>
                </label>
                <textarea v-model="feedback.feedbackText" 
                          rows="3"
                          class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
                          :placeholder="formData.newStatus === 'PAPARAN_COMPLETED' 
                            ? 'Jelaskan hasil evaluasi dari paparan, rekomendasi, atau catatan penting lainnya' 
            ...
            <button type="button" @click="addRequirementFeedback"
              ...
              <span v-if="formData.newStatus === 'PAPARAN_COMPLETED'">+ Tambah Feedback Paparan</span>
...
// Check if status needs requirement feedback - HOTFIX: Added PAPARAN_COMPLETED
const needsRequirementFeedback = (status: string) => {
  return ['INCOMPLETE_REQUIREMENTS', 'REVISION_REJECTED', 'PAPARAN_COMPLETED'].includes(status)
}
