<template>
      ...
      <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold text-slate-800">Dashboard Admin SLO</h1>
              <p class="mt-2 text-slate-600">Kelola semua permohonan Surat Layak Operasi (SLO).</p>
            </div>
        ...
        <div v-else-if="sloItems.length === 0" class="bg-white shadow-sm rounded-lg border border-slate-200 p-12 flex flex-col items-center">
          ...
          <h3 class="mt-4 text-lg font-medium text-slate-900">Tidak ada permohonan SLO</h3>
          <p class="mt-1 text-sm text-slate-500">Belum ada permohonan SLO yang sesuai dengan filter yang dipilih.</p>
        </div>
  ...
  // SEO metadata
  useHead({
    title: 'Admin Dashboard SLO | DLH Kabupaten Grobogan',
    meta: [
      { name: 'description', content: 'Dashboard administrator untuk mengelola semua permohonan Surat Layak Operasi (SLO) di Dinas Lingkungan Hidup Kabupaten Grobogan.' },
  ...
  const fetchSLOList = async () => {
    try {
      ...
      const response = await $fetch(`/api/slo/admin/list?${queryParams.toString()}`);
  ...
  const handleViewChange = (view) => {
    ...
    try {
      localStorage.setItem('admin-slo-dashboard-view', view);
    } catch (e) {
      // Ignore localStorage errors
    }
  };
  
  // View SLO detail (admin can manage)
  const viewSLODetail = (id) => {
    router.push(`/admin/slo/${id}`);
  };
  ...
  const exportData = async () => {
    try {
      ...
      console.log('Exporting SLO data...');
  ...
  const loadViewPreference = () => {
    try {
      const savedView = localStorage.getItem('admin-slo-dashboard-view');
