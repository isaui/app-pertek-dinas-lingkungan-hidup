<template>
  <div>
    <!-- SEO Head -->
    <Head>
      <Title>Detail SLO {{ slo?.sloNumber || sloId }} | DLH Kabupaten Grobogan</Title>
      <Meta name="description" content="Detail permohonan Surat Layak Operasi (SLO) Anda di DLH Kabupaten Grobogan" />
      <Meta name="keywords" content="slo, detail permohonan, status slo, dinas lingkungan hidup, grobogan" />
      <Meta property="og:title" content="Detail SLO | DLH Kabupaten Grobogan" />
      <Meta property="og:description" content="Pantau status dan kelola permohonan SLO Anda" />
    </Head>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-4">
                    ...
                    <div class="flex items-center text-sm text-slate-600">
                      ...
                      <span>{{ getSloTypeLabel(slo.sloType) }}</span>
                    </div>
                  </div>
                </div>
          ...
          <div class="bg-white shadow-sm rounded-lg border border-slate-200">
            ...
            <div class="p-6">
              ...
              <div v-show="activeTab === 'feedback'">
                <FeedbackTab 
                  :feedback="slo?.requirementFeedback || []"
                  :sloId="sloId"
                  @feedback-resolved="handleFeedbackResolved"
...
import RequirementsTab from '~/components/user/slo/RequirementsTab.vue';
...
const getActiveFeedback = () => {
  return slo.value.requirementFeedback?.filter(f => !f.isResolved) || [];
};

// Tab access control
const canUploadRevision = () => {
  return ['PAPARAN_COMPLETED', 'REVISION_REJECTED'].includes(slo.value.status);
};

const canUpdateRequirements = () => {
  return ['INCOMPLETE_REQUIREMENTS', 'REJECTED'].includes(slo.value.status);
};
...
const getSloTypeLabel = (type) => {
  const typeMap = {
    'AIR_LIMBAH': 'SLO Air Limbah',
    'PEMBUANGAN_EMISI': 'SLO Pembuangan Emisi'
  };
...
const fetchData = async () => {
  ...
  try {
    const response = await $fetch(`/api/slo/detail/${sloId}`);
...
const handleFeedbackResolved = (feedbackId) => {
  ...
  const feedback = slo.value.requirementFeedback?.find(f => f.id === feedbackId);
  if (feedback) {
...
const handleAllFeedbackResolved = (feedbackIds) => {
  ...
  if (slo.value.requirementFeedback) {
    slo.value.requirementFeedback.forEach(feedback => {
...
const getRevisionDocuments = () => {
  return slo.value.documents?.filter(doc => doc.type === 'REVISI') || [];
};

const getRequirementDocuments = () => {
  return slo.value.documents?.filter(doc => doc.type === 'PERSYARATAN') || [];
};
