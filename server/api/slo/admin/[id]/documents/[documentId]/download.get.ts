// server/api/slo/admin/[id]/documents/[documentId]/download.get.ts
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Get document and validate it belongs to the SLO
    const document = await prisma.sLODocument.findUnique({
      where: { id: documentId },
      include: {
        slo: {
          select: {
    ...
    if (document.sloId !== sloId) {
      throw createError({
        statusCode: 400,
        statusMessage: 'Document does not belong to this SLO'
      })
    }
    ...
    headers_response.set('X-SLO-Company', encodeURIComponent(document.slo.company))
